  ...
  <section className={`${styles.flexCenter} flex-row justify-center items-center   sm:pt-16 `}>
    {stats.map((stat) =>(
      <div key={stat.id} className={`flex-1  flex justify-center items-center flex-row m-3 mt-12`}>
        ...
        <p className='stats_hero font-poppins font-normal xs:text-[20px] text-[15px] xs:leading-[26px] leading-[21px] text-gradient uppercase ml-3 '>{currentLanguage === "en" ? stat.title : stat.titleSpanish}</p>
      </div>
    ))}
